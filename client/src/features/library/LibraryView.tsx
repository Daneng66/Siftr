...
export function LibraryView() {
  ...
  return (
    <div className="flex h-full flex-col">
      {/* Context / toolbar bar */}
      <div className="flex shrink-0 items-center gap-2 border-b border-slate-200 bg-white px-3 py-2 sm:gap-3 sm:px-5 sm:py-3 dark:border-slate-800 dark:bg-slate-900">
        <h1 className="text-base font-semibold sm:text-lg">{filterTitle(filter)}</h1>
        {!hardScanRunning && (
        ...
        {photos.length > 0 && (
          <button
            ...
            className="hidden text-xs text-brand-600 hover:underline sm:block sm:text-sm"
          >
            Select all loaded
          </button>
        )}
        <select
          ...
          className="rounded-lg border border-slate-200 bg-slate-50 px-2 py-1 text-xs outline-none focus:border-brand-400 sm:px-2.5 sm:py-1.5 sm:text-sm dark:border-slate-700 dark:bg-slate-800"
        >
          {SORTS.map((s) => (
